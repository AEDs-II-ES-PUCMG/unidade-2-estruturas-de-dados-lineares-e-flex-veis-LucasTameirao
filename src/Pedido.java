...
public class Pedido implements Comparable<Pedido> {
	...
	public Pedido(LocalDate dataPedido, int formaDePagamento) {
		...
		quantItensDePedido = itensDePedido.tamanho();
	...
	public ItemDePedido existeNoPedido(Produto produto) {
		...
		for (int i = 0; i < itensDePedido.tamanho(); i++) {
			if (itensDePedido.get(i).equals(itemDePedidoProcurado)) {
				return itensDePedido.get(i);
			}
		}
		return null;
	}
	...
	public boolean incluirProduto(Produto novo, int quantidade) {
		...
		if (itemDePedido != null) {
			...
			itensDePedido.inserirFinal(novoItem);
			return true;
		}
		return false;
	}
	...
	public String toString() {
		...
		for (int i = 0; i < itensDePedido.tamanho(); i++) {
			stringPedido.append(itensDePedido.get(i).toString() + "\n");
		}
